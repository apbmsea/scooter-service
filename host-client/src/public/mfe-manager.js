class MicrofrontendManager {
    ...
    async init() {
        ...
        if (response.ok) {
            ...
            this.preloadAllMicrofrontends().catch(err => {
                console.warn('Ошибка предзагрузки микрофронтендов при инициализации', err);
            });
        }
    }
    ...
    async updateMicrofrontends(pageConfig) {
        ...
        const toUnmount = currentlyMounted.filter(mfe => !requiredMFEs.includes(mfe));
        const toMount = requiredMFEs.filter(mfe => !currentlyMounted.includes(mfe));
    ...
    async loadCSS(href, name) {
        return new Promise((resolve, reject) => {
            const existingLink = document.querySelector(`link[data-mfe="${name}"]`);
            if (existingLink) {
                resolve();
                return;
            }
            ...
            link.onload = () => resolve();
            link.onerror = () => reject(new Error(`Ошибка загрузки CSS ${name}`));
    ...
    async loadJS(src, name) {
        return new Promise((resolve, reject) => {
            ...
            script.onerror = () => reject(new Error(`Ошибка загрузки JS ${name}`));
        ...
        if (mountFunction && typeof mountFunction === 'function') {
            ...
            const checkInterval = setInterval(() => {
                ...
                if (fn && typeof fn === 'function') {
                    ...
                    clearInterval(checkInterval);
                    console.warn(`Функция монитрования ${mountFunctionName} не найденна для ${name} после ${maxAttempts} попыток`);
                }
            }, 100);
        }
    }
